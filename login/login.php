...
<?
  ...
  @extract($_SESSION); 
   ...
   $result = mysql_query($sql, $connect); //검색한 아이디가 있으면 1 / 없으면 null
   ...
   if(!$num_match) //검색된 레코드가 없으면
   {
    ...
    else    //검색된 레코드가 있으면
    {
		 $row = mysql_fetch_array($result); 
          //$row[id] ,.... $row[level]
         $sql ="select * from member where id='$id' and pass=password('$pass')"; //password()함수로 암호화된 텍스트는 암호화되기 전 텍스트로 돌릴수 없음
        ...
        else    // 입력 pass 와 테이블에 저장된 pass 일치한다.  id와 password가 모두 일치하면
        {
           $userid = $row[id];
		   $username = $row[name];
		   $usernick = $row[nick];
